-- 经销商积分明细表
CREATE TABLE `account_dealer_integral_detail` (
  ...
  `integral_handle_type` varchar(20) DEFAULT NULL COMMENT '操作类型（INCREASE-增加，DECREASE-减少）',
  ...
  KEY `idx_integral_handle_type` (`integral_handle_type`),
  KEY `idx_handle_date_time` (`handle_date_time`),
  KEY `idx_audit_status` (`audit_status`),
  KEY `idx_create_time` (`create_time`),
  KEY `idx_deleted` (`deleted`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='经销商积分明细表';

-- 添加表注释
ALTER TABLE `account_dealer_integral_detail` COMMENT '经销商积分明细表';